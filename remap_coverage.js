...
_.forIn(originalCoverageJson, (value, key) => {
  const updatedKey = key.replace(path.normalize('/src/'), path.normalize('/dist/')).replace('.ts', '.js');
...
writeReport(collector, 'json', {}, './coverage/coverage_remapped/coverage.json');
writeReport(collector, 'lcovonly', {}, './coverage/coverage_remapped/coverage.lcov');
writeReport(collector, 'html', {}, './coverage/coverage_remapped/html');